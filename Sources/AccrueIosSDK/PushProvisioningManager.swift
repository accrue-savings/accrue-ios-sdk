...
final class AppleWalletPushProvisioningManager: NSObject, PKAddPaymentPassViewControllerDelegate {
    ...
    func start(from webView: WKWebView, with config: [String: String]) {
        DispatchQueue.main.async { [weak self] in
            ...
            guard let cardholder = config["cardholderName"]?.trimmingCharacters(in: .whitespacesAndNewlines),
                  let suffix = config["cardSuffix"]?.trimmingCharacters(in: .whitespacesAndNewlines),
                  let desc = config["description"]?.trimmingCharacters(in: .whitespacesAndNewlines),
                  !cardholder.isEmpty, !suffix.isEmpty, !desc.isEmpty else {
    ...
    func addPaymentPassViewController(_ controller: PKAddPaymentPassViewController,
        ...
        do {
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                let script = """
                window.dispatchEvent(new CustomEvent('\(AccrueWebEvents.AppleWalletProvisioningSignRequest)', { 
                    detail: \(jsonString) 
                }));
                """
                ...
                webView.evaluateJavaScript(script) { result, error in
                    if let error = error {
                        logger.error("JS evaluation error: \(error.localizedDescription, privacy: .public)")
    ...
    func addPaymentPassViewController(_ controller: PKAddPaymentPassViewController,
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            let script = """
            window.dispatchEvent(new CustomEvent('\(AccrueWebEvents.AppleWalletProvisioningResult)', { 
                detail: { 
                    success: \(success), 
    ...
    func handleBackendResponse(rawJSON: String) {
        ...
        do {
            ...
            guard let dict = try JSONSerialization.jsonObject(with: data) as? [String: String],
                  ...
                  let ephemeralPublicKey = dict["ephemeralPublicKey"] else {
            ...
            guard let activation = Data(base64Encoded: activationData),
                  ...
                  let ephemeral = Data(base64Encoded: ephemeralPublicKey) else {
